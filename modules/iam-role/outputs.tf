...
output "codebuild_role_arn" {
  value       = var.create_new_role ? aws_iam_role.codebuild_role.arn : data.aws_iam_role.existing_codebuild_role[0].arn
  description = "The ARN of the IAM Role"
}

output "codebuild_role_name" {
  value       = var.create_new_role ? aws_iam_role.codebuild_role.name : data.aws_iam_role.existing_codebuild_role[0].name
  description = "The ARN of the IAM Role"
}
